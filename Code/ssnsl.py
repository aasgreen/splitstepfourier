...
if __name__ == '__main__':
    ...
    #Plot Spectrum
    spec =np.fft.fftshift( np.fft.fft(uu))
    ...
    plt.xlabel('Time')
    plt.subplot(2,1,2)
    plt.plot(np.fft.fftshift(np.fft.fftfreq(param.nt,d=param.dtau)), np.absolute(spec*param.dtau/np.sqrt(2*np.pi))**2, 'ko-')
    plt.xlim([-.5,.5])
    plt.title('Power of Spectrum')
    plt.xlabel('Frequency')
    #Store Dispersion
    dispersion = np.exp(1j*0.5*param.beta2*np.fft.ifftshift(freq)**2*param.deltaz)
   ...
   # print 'test7'
    ...
    plt.subplot(2,1,2)
    plt.plot(freq,np.abs(A[-1,:]*param.dtau/np.sqrt(2*np.pi))**2)
    plt.xlim([-.5,.5])
    plt.ylim([0,2])
    plt.subplot(2,1,1)
    plt.plot(tau, np.abs(Wt[-1,:])**2)
    plt.xlim([-5,5])
    print 'test9'
    #f.savefig('nzero.eps')    
