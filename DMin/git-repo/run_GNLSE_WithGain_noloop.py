...
# Format is frequency (Hz) | Intensity

baseDir='O:\\OFM\\Maser\\PM for OPO and DFG'
#pulseData = np.genfromtxt(str(baseDir+'\\pulseEnteringEDF.csv'),delimiter=',')

'''changing the input spectrum'''

pulseData = np.genfromtxt('O:\\OFM\\Maser\\FROG\\100MHz-comb-50cm-smf-28\\Speck.dat')
pulseData = pulseData[:,:3]
c0 = 299792458 #m/s
pulseData[:,0] = c0/(pulseData[:,0]*1e-9) - c0/(wavelength*1e-9)
...
phaseInterp = -1*phaseFunc(V*1e12/(2*np.pi))
...
#for fiberlengthPANDAinitial in np.arange(0.1, 0.4, 0.05):
#
#    for fiberlength in np.arange(0.7, 1.3, 0.05):

# Gain fiber parameters

fiberlengthPANDAinitial = 1.05 - 0.5
fiberlength = 1.3 #EDF

gamma = 5e-3 # 1/W/m
#fiberlength = 1.0 # fiber length (m)
gain = 17.5/fiberlength # dB/m, scaled

# Compression fiber parameters
gammaPANDA  = 1.1e-3 # 1/W/m
gainPANDA   = 0.0 # dB/m
fiberlengthPANDA = 0.6

# Raman response
fr     = 0.18 # fractional Raman contribution
tau1   = 0.0122 
tau2   = 0.032
RT     = (tau1**2 + tau2**2) /tau1 / tau2**2*np.exp(-np.abs(T)/tau2)*np.sin(T/tau1)
RT[T<0]=0 # Raman response only after impulse
RT     = RT / np.trapz(RT, x=T) # Normalize to unit integral
...
#Run simulation for initial compression fiber

[Zinitial, ATinitial, AWinitial, Winitial] = gnlsewithgain(T,fftpack.fftshift(A),w0,gammaPANDA,betasPANDA,gainPANDA,fr,RT,fiberlengthPANDAinitial,nsaves,isgain=False)
AtoFiber = ATinitial[len(ATinitial[:,0]) - 1,:]

# Run simulation for amplifier
[Z, AT, AW, W] = gnlsewithgain(T,fftpack.fftshift(AtoFiber),w0,gamma,betas,gain,fr,RT,fiberlength,nsaves,isgain=True)
AFinal = AT[len(AT[:,0]) - 1,:]

# Run simulation for compression fiber
[ZC, ATC, AWC, WC] = gnlsewithgain(T,fftpack.fftshift(AFinal),w0,gammaPANDA,betasPANDA,gainPANDA,fr,RT,fiberlengthPANDA,nsaves,isgain=False)

print 'Pulse energy before amplifier is ',1e9*sum(dTSeconds*abs(A)**2),' nJ'
...
preSMF = len(Zinitial) + len(Z)        

Z =  np.hstack((Zinitial, max(Zinitial)+Z, max(Z)+max(Zinitial)+ZC))
AT =  np.vstack((ATinitial, AT, ATC))
AW =  np.vstack((AWinitial, AW, AWC))
...
np.savetxt('.\\Fiber Length Optimization Output\\Z_SMF' + str(fiberlengthPANDAinitial) + 'EDF' + str(fiberlength) + 'new.csv', Z[preSMF:], delimiter=',')
np.savetxt('.\\Fiber Length Optimization Output\\LIT_SMF' + str(fiberlengthPANDAinitial) + 'EDF' + str(fiberlength) + 'new.csv', lIT[preSMF:,:], delimiter=',')
...
z0 = 0.4+fiberlengthPANDAinitial+fiberlength # m
err = min(abs(Z-z0))
...
plt.xlim([-500,500])
#np.savetxt('.\\GNLSEModeledCompressedPulseNoGainNonlinearity_SMF' + str(fiberlengthPANDAinitial) + 'EDF' + str(fiberlength) + '.csv', np.transpose([1000*(T - T[peakIdx - 1]), (abs(AT[index, :]**2)[::-1])]), delimiter=',')
